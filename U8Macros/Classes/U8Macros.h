...
//  Pods
//
//  Created by upping8 on 2022/10/29.
//

#ifndef U8Macros_h
#define U8Macros_h

#import "U8MetaMacros.h"
...
#define onExit \
    u8_keywordify \
    __strong u8_cleanupBlock_t metamacro_concat(u8_exitBlock_, __LINE__) __attribute__((cleanup(u8_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    u8_keywordify \
    metamacro_foreach_cxt(u8_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    u8_keywordify \
    metamacro_foreach_cxt(u8_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(u8_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^u8_cleanupBlock_t)(void);

static inline void u8_executeCleanupBlock (__strong u8_cleanupBlock_t *block) {
    (*block)();
}

#define u8_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define u8_strongify_(INDEX, VAR) \
...
// autorelease pools.
#if DEBUG
#define u8_keywordify autoreleasepool {}
#else
#define u8_keywordify try {} @catch (...) {}
#endif
